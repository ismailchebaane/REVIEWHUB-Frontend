...
export default function Login({ location }) {
    ...
    const [alert,setalertd]=useState("")
   ...
   try {
     await    axios.post("https://reviewhub-backendf.onrender.com/api/auth/login",{
             email:email,
             password:password
        }) .then( (res)=> {
...
<div  className={alertfalse?"alert alert-error  flex justify-between absolute top-[20%] z-[999] max-w-sm  shadow-lg":"hidden"}>
  <div>
    ...
    <span> Wrong {alert}! try again.</span>
  </div>
  ...
  <div  class="  lg:h-max md:h-max  sm:h-full  py-5 lg:w-full  p-3   container flex flex-column justify-center align-center   " >
    ...
    <div>
      ...
      <div class="flex my-[20px] flex-col ">
        ...
        <input class="flex justify-center items-center mt-2 text-center self-center rounded-2xl p-2 bg-gray-100 focus:outline-0 lg:w-full md:w-full w-[250px]" type="email" name="email"  autocomplete="false"  required  placeholder="example@gmail.com"
      ...
      <div class="flex my-[20px] flex-col  ">
        ...
        <h1 className="text-gray-900 mt-4 cursor-pointer">Forget password?</h1>
      </div>
